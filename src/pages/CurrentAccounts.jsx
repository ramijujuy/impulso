...
const CurrentAccounts = () => {
      ...
      if (res.data.success) {
        ...
        if (personId) {
          accountsData = accountsData.filter(
            (acc) =>
              acc.accountType === "person" &&
              acc.person &&
              acc.person._id === personId
          );
        }
  ...
  const fetchCollections = async () => {
    ...
    try {
      ...
      const res = await axios.get("/api/current-accounts/collections", {
        params,
      });
      if (res.data.success) {
  ...
  const handleViewGroupDetails = (groupAccount) => {
    ...
    const members = accounts.filter(
      (acc) =>
        acc.accountType === "person" && acc.person?.group?._id === groupId
    );
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setReceiptData({
          ...
          amount:
            selectedInstallment.amount - (selectedInstallment.amountPaid || 0),
          payerName: selectedAccount.person
            ? selectedAccount.person.fullName
  ...
  const getFilteredInstallments = (account) => {
    ...
    return account.installments.filter((inst) => {
      ...
      if (filterDate === "thisMonth") {
        return (
          dueDate >= startMonth && dueDate <= endMonth && inst.status !== "paid"
        );
      }
  ...
  const handleExport = () => {
    ...
    if (activeTab === "accounts") {
      accounts.forEach((acc) => {
        ...
        // Apply group filter
        if (filterGroup !== "all") {
          if (acc.accountType === "group" && acc.group?._id !== filterGroup)
            return;
          if (
            acc.accountType === "person" &&
            acc.person?.group?._id !== filterGroup
          )
            return;
        }
      ...
      XLSX.writeFile(
        {
          SheetNames: ["Cuentas"],
          Sheets: { Cuentas: XLSX.utils.json_to_sheet(dataToExport) },
        },
        `cuentas_${viewType === "person" ? "individuales" : "grupales"}.xlsx`
      );
    } else {
      // Export collections
      ...
      XLSX.writeFile(
        {
          ...
          Sheets: { Cobranzas: XLSX.utils.json_to_sheet(dataToExport) },
        },
        "cobranzas.xlsx"
      );
    }
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Gestione las cuentas corrientes y registre los pagos.
          </p>
        </div>
        ...
        <>
          <div>
            <div className="mb-4 flex space-x-4 border-b border-gray-200 pb-2">
              <button
                className={`pb-2 px-1 text-sm font-medium transition relative ${
                  viewType === "person"
          ...
          <div className="grid gap-6">
            {loading ? (
              ...
              accounts.map((acc) => {
                ...
                const visibleInstallments = getFilteredInstallments(acc);
                if (visibleInstallments.length === 0 && filterDate !== "all")
                  return null;
                      ...
                      {visibleInstallments.length > 0 ? (
                        visibleInstallments.map((inst) => (
                          <div
                            ...
                            className={`p-4 flex flex-col sm:flex-row justify-between items-center transition gap-4 ${
                              inst.status === "paid"
          ...
          <div className="space-y-6">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
              ...
              <p className="text-blue-600 text-xs mt-1">
                Cuota #{selectedInstallment?.installmentNumber} • Vencimiento:{" "}
          ...
          <div className="space-y-4">
            <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100">
              <p className="text-yellow-800 text-sm">
                Editando cuota #{selectedInstallment?.installmentNumber} de{" "}
                <strong className="font-semibold">
                  {selectedAccount?.person?.fullName ||
                    selectedAccount?.group?.name}
                </strong>
            ...
            <div className="flex justify-end space-x-3 pt-2">
              ...
              <button onClick={confirmEditDate} className="btn-primary">
                Guardar Cambios
              </button>
            </div>
          </div>
      ...
      {showGroupDetailsModal && (
        <Modal
          title={`Detalle de Grupo: ${selectedAccount?.group?.name}`}
          onClose={() => setShowGroupDetailsModal(false)}
        >
          <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
            <p className="text-sm text-gray-500 mb-4">
              Estado de cuentas individuales de los miembros.
            </p>
            {selectedGroupMembers.length > 0 ? (
              <div className="space-y-3">
                {selectedGroupMembers.map((memberAcc) => {
                  const pending = memberAcc.installments.filter(
                    (i) => i.status !== "paid"
                  ).length;
                  const overdue = memberAcc.installments.filter(
                    (i) =>
                      i.status !== "paid" && new Date(i.dueDate) < new Date()
                  ).length;
                  const totalDebt = memberAcc.installments
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-bold text-gray-900">
                            {memberAcc.person.fullName}
                          </h4>
                          <p className="text-xs text-gray-500">
                            DNI: {memberAcc.person.dni}
                          </p>
                        </div>
                        <div className="text-right">
                      ...
                      <details className="mt-2">
                        ...
                        <div className="mt-2 space-y-1">
                          {memberAcc.installments.map((inst) => (
                            <div
                              key={inst.installmentNumber}
                              className="flex justify-between text-xs border-b border-gray-100 py-1"
                            >
                              <span>
                                #{inst.installmentNumber} -{" "}
                                {new Date(inst.dueDate).toLocaleDateString()}
                              ...
                              <div className="text-right">
                                ...
                                >
                                  {inst.status === "paid"
                                    ...
                                    : inst.status === "partial"
                                    ? `Parcial ($${
                                        inst.amountPaid?.toLocaleString() || 0
                                      })`
                                    : inst.status === "overdue"
                                    ? "Vencida"
                                    : "Pendiente"}
              ...
              <p className="text-center text-gray-500 py-4">
                No se encontraron miembros con cuenta activa.
              </p>
            )}
